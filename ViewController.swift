...
class ViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    func generateRandomRevenue() -> Double {
           let numberArray = [119900, 139900, 79900, 89900]
           return Double(numberArray.randomElement() ?? 0)
    }
    ...
    @objc func sendPurchaseEvent(_ sender: UIButton) {
        ...
        event.currency = .USD
        event.transactionID = generateRandomAlphanumericString(length: 5)
        event.revenue = NSDecimalNumber(value: generateRandomRevenue())
